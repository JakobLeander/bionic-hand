...
class scscl(protocol_packet_handler):
    ...
    def WritePosition(self, scs_id, position):
        txpacket = [
    ...
    def WriteSpeed(self, scs_id, speed):
        txpacket = [
            self.scs_lobyte(speed),
            self.scs_hibyte(speed),
        ]
        return self.writeTxRx(scs_id, SCSCL_GOAL_SPEED_L, len(txpacket), txpacket)

    def WriteAcceleration(self, scs_id, acceleration):
        txpacket = [
            self.scs_lobyte(acceleration),
            self.scs_hibyte(acceleration),
        ]
        return self.writeTxRx(scs_id, SCSCL_GOAL_TIME_L, len(txpacket), txpacket)

    def ReadPosition(self, scs_id):
